 ...
 * This program allows the user to "pop" an integer off the top of the stack and add values to a stack, look at the top value or clear the stack
...
public class StackProgram {
	...
	private void initialize() {
		...
		frmStackCrashProof.setBounds(100, 100, 450, 331);
		...
		lstStackValue.setBounds(65, 106, 281, 175);
		...
		btnAddValue.setBounds(59, 42, 132, 23);
		...
		JButton btnClearStack = new JButton("Clear Stack");
		btnClearStack.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				//clear the list
				model.clear();
				...
				if(myStack.isEmpty()) {
					//display a message saying there is no numbers in the stack
					JOptionPane.showMessageDialog(null, "You can only clear the stack if the stack is not empty.");
				}
				else {
					//remove the number from the stack and get the stack
					myStack = stacks.Clear();
		...
		btnClearStack.setFont(new Font("Tahoma", Font.PLAIN, 14));
		btnClearStack.setBounds(212, 75, 134, 23);
		frmStackCrashProof.getContentPane().add(btnClearStack);
		...
		btnPeek.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(myStack.isEmpty()) {
					...
					//display the top number to the user
					JOptionPane.showMessageDialog(null, "The top number of the stack is " + myInt + ".");
					
				}
				
			}
		});
		btnPeek.setFont(new Font("Tahoma", Font.PLAIN, 14));
		btnPeek.setBounds(57, 76, 134, 23);
		frmStackCrashProof.getContentPane().add(btnPeek);
